...
// total movies names
// movies.map(m=>m.title).forEach(n=>console.log(n))

// print comedy genres movies
// movies.filter(m=>m.genres=="Comedy").forEach(m=>console.log(m.title))
// var cmovies=movies.filter(m=>m["genres"].includes("Crime"))
// console.log(cmovies);

// movies.filter(m=>m.genres=="Drama").forEach(m=>console.log(m.title))

// highest runtime
// var highest=movies.reduce((m1,m2)=>m1.runtime>m2.price?m1:m2)
// console.log(highest);

// sort movies in ascending order based on year
// movies.sort((m1,m2)=>m1.year-m2.year)
// console.log(movies);

// var isPresent=movies.some(m=>m.title=="avatar")
// console.log(isPresent);

// console.log(movies.find(m=>m.id==10));

// yearcount
// var yc={}
// movies.map(m=>m.year in yc ? yc[m.year]+=1 :yc[m.year]=1)
// console.log(yc);

// sort movies runtime
// movies.sort((m1,m2)=>m1.runtime - m2.runtime)
// movies.forEach(m=>console.log(m.runtime))

// print highest year
var yc={}
movies.map(m=>m.year in yc ? yc[m.year]+=1 :yc[m.year]=1)
console.log(yc);
var out=Object.entries(yc).sort((o1,o2)=>o2[1]-o1[1])
console.log(out);

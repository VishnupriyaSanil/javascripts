...
var allCategories=new Set(data.map(p=>p.category))
...
function populateCategories(categories){
    ...
    categories.forEach(c=>{
        htmlData+=`
        <div class="p-2"><button class="btn btn-outline-light " name="${c}"onclick="fetchProductsByCategoryName(event)">${c}</button>
        </div>`
    })
    document.querySelector("#id_nav").innerHTML=htmlData
}

function fetchProductsByCategoryName(event){
